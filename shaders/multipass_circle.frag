...
void main() {
    ...
    float MBH = MBlackHole; // 使用uniform传入的值
    ...
    while(flag==true){//直线传播raymarching
        lastRayPos = RayPos;
        lastRayDir = RayDir;
        lastR = Dis;
        ...
        */
        ...
        RayDir = normalize(RayDir+(dthe+dthe*dthe*dthe/3.0)*cross(cross(RayDir,NPosToBH),RayDir)/costheta);//更新方向
        */
        ...
        //                     WorldZ, BHRPos, BHRDiskDir, Rs, RIn, ROut, diskA, TPeak4, shiftMax);
        
        count++;
        if(Dis > (100.*Rs) && Dis > lastR && count > 50) {
            flag = false;
