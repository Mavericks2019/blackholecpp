...
MainWindow::~MainWindow() {
    // Clean up resources - 新增清理lensing相关资源
    delete basicCanvas;
    delete basicControl;
    delete circleCanvas;
...
void MainWindow::closeEvent(QCloseEvent* event) {
    // Clean up OpenGL resources - 新增清理lensing资源
    if (basicCanvas) {
...
void MainWindow::showEvent(QShowEvent* event) {
    ...
    // Force repaint of all canvases - 新增lensingCanvas
    if (basicCanvas) {
        basicCanvas->update();
    }
    if (circleCanvas) {
...
void MainWindow::onTabChanged(int index) {
    ...
    // Force repaint when switching tabs - 新增lensingCanvas处理
    switch(index) {
        case 0: // Black Hole
...
// 在 createTabs 函数中修改Tab标题
void MainWindow::createTabs() {
    // Black Hole tab (index 0)
...
void MainWindow::createControlPanels() {
    ...
    // Multi-Pass control panel (index 3)
...
void MainWindow::applyStyles() {
    ...
    // 新增：2D引力透镜标题样式
    QLabel* lensingTitle = lensingControl->findChild<QLabel*>("titleLabel");
    if (lensingTitle) {
        lensingTitle->setStyleSheet("font-size: 18px; font-weight: bold; color: #d0d0ff; padding: 15px 0;");
    }
    ...
    // 新增：2D引力透镜按钮样式
    QList<QPushButton*> lensingButtons = lensingControl->findChildren<QPushButton*>();
    for (QPushButton* btn : lensingButtons) {
        btn->setStyleSheet(
            "QPushButton {"
            "   background-color: #5a5a7a; color: #e0e0ff; border: 1px solid #787898;"
            "   border-radius: 5px; padding: 8px; font-weight: bold; font-size: 12px; min-height: 30px;"
    ...
    // 新增：2D引力透镜页脚样式
    QLabel* lensingFooter = lensingControl->findChild<QLabel*>();
    if (lensingFooter && lensingFooter->text().contains("©")) {
        lensingFooter->setStyleSheet("color: #9090a0; font-size: 10px; margin-top: 20px;");
    }
