...
// Helper lambda to load a shader from Qt resource
auto loadShader = [](QOpenGLShaderProgram* prog, QOpenGLShader::ShaderType type, const QString& path) -> bool {
    QFile file(path);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qWarning() << "Could not open shader file:" << path;
        return false;
    }
    QByteArray source = file.readAll();
    file.close();
    if (!prog->addShaderFromSourceCode(type, source)) {
        qWarning() << "Failed to compile shader:" << path << prog->log();
        return false;
    }
    return true;
};
...
void GLCircleWidget::initializeGL() {
    ...
    loadShader(program, QOpenGLShader::Vertex, ":/shaders/circle.vert");
    loadShader(program, QOpenGLShader::Fragment, ":/shaders/circle.frag");
    if (!program->link()) {
    ...
    loadShader(screenProgram, QOpenGLShader::Vertex, ":/shaders/screen.vert");
    loadShader(screenProgram, QOpenGLShader::Fragment, ":/shaders/screen.frag");
    ...
    loadShader(mipmapProgram, QOpenGLShader::Vertex, ":/shaders/screen.vert");
    loadShader(mipmapProgram, QOpenGLShader::Fragment, ":/shaders/mipmap.frag");
    ...
    loadShader(horizontalProgram, QOpenGLShader::Vertex, ":/shaders/screen.vert");
    loadShader(horizontalProgram, QOpenGLShader::Fragment, ":/shaders/horizontal.frag");
    ...
    loadShader(verticalProgram, QOpenGLShader::Vertex, ":/shaders/screen.vert");
    loadShader(verticalProgram, QOpenGLShader::Fragment, ":/shaders/vertical.frag");
    if (!verticalProgram->link()) {
        qDebug() << "Vertical shader link error:" << verticalProgram->log();
    }

    // Create result shader program
    resultProgram = new QOpenGLShaderProgram(this);
    loadShader(resultProgram, QOpenGLShader::Vertex, ":/shaders/screen.vert");
    loadShader(resultProgram, QOpenGLShader::Fragment, ":/shaders/screen_result.frag");
    if (!resultProgram->link()) {
        qDebug() << "Result shader link error:" << resultProgram->log();
    }
